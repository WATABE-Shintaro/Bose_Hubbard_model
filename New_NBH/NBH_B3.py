...
class MyModel(nn.Module):
    def __init__(self):
        super().__init__()
        # 全結合ネットワーク
        self.l1 = nn.Linear(LATTICE, net_num1)
        self.l2 = nn.Linear(net_num1, 2)
...
def learning():
    ...
    with torch.no_grad():
        ...
        #aaa = forward(est_n_vector_array, cn_array, cn_tensor_1, cn_tensor_2)
...
"""
def forward(n_vector_array, cn_array, cn_tensor_1, cn_tensor_2):
    i_vector_array = [np.arange(LATTICE) for i in range(SAMPLE_NUM)]
    ...
    cn_tensor_1 = cn_tensor_1[:, :, 0] * torch.exp(cn_tensor_1[:, :, 1] * PI * 1j)
    cn_tensor_2 = cn_tensor_2[:, :, 0] * torch.exp(cn_tensor_2[:, :, 1] * PI * 1j)
    e = torch.abs(cn_array) ** 2
    d = torch.sum(e)
    a = -J * torch.sum(torch.sqrt(n_vector_array[:, 0:LATTICE - 1] * (n_vector_array[:, 1:LATTICE] + 1)) *
                               torch.conj(cn_tensor_1) * torch.reshape(cn_array, (-1, 1))
                               + torch.sqrt((n_vector_array[:, 0:LATTICE - 1] + 1) * n_vector_array[:, 1:LATTICE]) *
                               torch.conj(cn_tensor_2) * torch.reshape(cn_array, (-1, 1))) / d
    b = J * torch.sum((i_vector_array ** 2 - (LATTICE - 1) * i_vector_array + (
                        LATTICE - 1) ** 2 / 4) * n_vector_array * torch.reshape(
                    torch.abs(cn_array) ** 2 ,(-1, 1))) / d
    b2 = J * torch.sum(
        (i_vector_array  - (LATTICE - 1) / 2) ** 2 * n_vector_array * torch.reshape(torch.abs(cn_array) ** 2,(-1, 1)) ) / d                                                                    
    c =  U / 2 * torch.sum((n_vector_array ** 2 - n_vector_array) * torch.reshape(
                    torch.abs(cn_array) ** 2 ,(-1, 1))) / d
