...
class MyLoss(nn.Module):
    ...
    def forward(self, n_vector_array, cn_array, cn_tensor_1, cn_tensor_2, n_correction_array):
...
def montecarlo_correction(n_vector_array, sample_num):
    n_correction_array = np.ones(sample_num)
    for i in range(sample_num):
        correction = 1
        for j in range(1, LATTICE - 1):
            if n_vector_array[i, j] == 0:
                correction += 1
            else:
                n_correction_array[i] *= math.factorial(correction)
                correction = 1
        n_correction_array[i] *= math.factorial(correction)
    n_correction_array = torch.from_numpy(n_correction_array).float()
    n_correction_array = n_correction_array.to(DEVICE)
    return n_correction_array
...
def learning():
    ...
    train_n_correction_array = montecarlo_correction(train_n_vector_array, SAMPLE_NUM)
    ...
    for e in range(EPOCH):
        ...
        my_net.train(True)
        for tr_n_v_a, tr_n_v_t_1, tr_n_v_t_2, tr_n_c_a in train_dataloader:
            optimizer.zero_grad()
            cn_array = my_net(tr_n_v_a)
            cn_tensor_1 = my_net(tr_n_v_t_1)
            cn_tensor_2 = my_net(tr_n_v_t_2)
            energy = criterion(tr_n_v_a, cn_array, cn_tensor_1, cn_tensor_2, tr_n_c_a)
            energy.backward()
            optimizer.step()
        ...
        my_net.eval()
        with torch.no_grad():
            for te_n_v_a, te_n_v_t_1, te_n_v_t_2, te_n_c_a in test_dataloader:
                cn_array = my_net(te_n_v_a)
                cn_tensor_1 = my_net(te_n_v_t_1)
                cn_tensor_2 = my_net(te_n_v_t_2)
                energy = criterion(te_n_v_a, cn_array, cn_tensor_1, cn_tensor_2, te_n_c_a)
    ...
    with torch.no_grad():
        ...
        est_n_correction_array = montecarlo_correction(est_n_vector_array, SAMPLE_NUM)
        cn_array = my_net(est_n_vector_array)
        cn_tensor_1 = my_net(est_n_vector_tensor_1)
        cn_tensor_2 = my_net(est_n_vector_tensor_2)
        energy = criterion(est_n_vector_array, cn_array, cn_tensor_1, cn_tensor_2, est_n_correction_array)
